...
loading_animation() {
    ...
    while ps -p $pid > /dev/null; do
        ...
        printf "\e[33mSedang Melakukan Instalasi, Mohon Bersabar\e[0m"
        printf "\n"
        printf "\e[1G\e[2K" # Kembali ke awal baris dan hapus baris
        printf "\e[32m["
        for ((i = 0; i < 20; i++)); do
            printf "%s" "${bars_chars[$bars_index]}"
            bars_index=$(( (bars_index + 1) % $num_bars ))
            sleep $delay
        done
        printf "]\e[0m\n"
        sleep $delay
...
echo -e "\e[32mloading...\e[0m" 
clear
izinsc="https://raw.githubusercontent.com/Eritsuu/joy/main/daftar"
# USERNAME
rm -f /usr/bin/user
